...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        if (users[i].countMutual(getUser(name)) > countMax) {
            best = users[i].getName();
            countMax = users[i].countMutual(getUser(name));
        }
      }                          
        return best;
    }
    ...
    public String mostPopularUser() {
        ...
        String max = null;
        for (int i = 0; i < userCount; i++) {
            if (followeeCount(users[i].getName()) > countMax) {
                countMax = followeeCount(users[i].getName());
                max = users[i].getName();
            }
        }
        return max;
